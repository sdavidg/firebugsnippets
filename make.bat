...
if not exist %_7zip% set _7zip="%programfiles(x86)%\7-Zip\7z.exe"
...
rem set _files=install.rdf *.manifest *.js *.jsm *.xul *.xml *.html *.css license* *.png defaults modules components locale chrome idl
set _files=bootstrap.js chrome.manifest install.rdf license* locale chrome
if exist %_7zip% (
	echo =^> %_7zip%
